...
def advance_simulation():
    ...
    sim_multiplier = 4.0
    # Clamp real_elapsed to 1.0s max to prevent massive jumps if server was idle
    elapsed = min(1.0, real_elapsed) * sim_multiplier
    ...
    for site_name, site_data in live_state["sites"].items():
        ...
        for w in site_data["workers"]:
            ...
            w["oxygen_percent"] -= 0.002 * elapsed # Baseline degradation (much slower)
...
def load_scenario(name: str = "gas_leak", site: str = "Site A"):
    ...
    sim_engine.sites_state[site]["gas_heatmap"] = [[0.0 for _ in range(10)] for _ in range(8)]
    sim_engine.sites_state[site]["collapsed_cells"] = []
    live_state["sites"][site]["collapsed_cells"] = []
    sim_engine.sites_state[site]["gas_origins"] = []
...
def send_command(worker_id: str, command: str, site: str = "Site A"):
    ...
    if command == "EVACUATE":
        ...
        prioritized_worker = compute_rescue_priorities([copy.deepcopy(worker)])[0]
        survival_prob = prioritized_worker.get("survival_probability", 1.0)
        
        if survival_prob < 0.4 or worker.get("oxygen_percent", 20.0) < 16.0:
            condition = "Critical/Severe"
            level = "error" # Will show up vividly in logs
        elif survival_prob < 0.7 or worker.get("gas_ppm", 0) > 50 or worker.get("heart_rate", 80) > 120:
            condition = "Hurt/Need Help"
            level = "warning"
        else:
